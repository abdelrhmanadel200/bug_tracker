<footer class="bg-light mt-5 py-4">
    <div class="container">
        <div class="row">
            <div class="col-md-4">
                <h5><i class="fas fa-bug text-primary me-2"></i>BugTracker</h5>
                <p class="small">A comprehensive bug tracking solution for software development teams.</p>
            </div>
            <div class="col-md-4">
                <h5>Quick Links</h5>
                <ul class="list-unstyled">
                    <li><a href="index.php" class="text-decoration-none">Home</a></li>


                    <?php if (empty($_SESSION['user_id'])): ?>
                        <li><a href="features.php" class="text-decoration-none">Features</a></li>
                    <?php else: ?>
                        <li><a href="../features.php" class="text-decoration-none">Features</a></li>
                    <?php endif; ?>

                    <?php if (empty($_SESSION['user_id'])): ?>
                        <li><a href="about.php" class="text-decoration-none">About</a></li>
                    <?php else: ?>
                        <li><a href="../about.php" class="text-decoration-none">About</a></li>
                    <?php endif; ?>

                    <?php if (empty($_SESSION['user_id'])): ?>
                        <li><a href="contact.php" class="text-decoration-none">Contact</a></li>
                    <?php else: ?>
                        <li><a href="../contact.php" class="text-decoration-none">Contact</a></li>
                    <?php endif; ?>
                </ul>
            </div>
            <div class="col-md-4">
                <h5>Contact Us</h5>
                <p class="small">Email: support@bugtracker.com<br>Phone: +1 (123) 456-7890</p>
                <div class="social-icons">
                    <a href="#" class="text-decoration-none me-2"><i class="fab fa-facebook"></i></a>
                    <a href="#" class="text-decoration-none me-2"><i class="fab fa-twitter"></i></a>
                    <a href="#" class="text-decoration-none me-2"><i class="fab fa-linkedin"></i></a>
                    <a href="#" class="text-decoration-none"><i class="fab fa-github"></i></a>
                </div>
            </div>
        </div>
        <div class="row mt-3">
            <div class="col-12 text-center">
                <p class="small text-muted">© 2025 BugTracker. All rights reserved.</p>
            </div>
        </div>
    </div>
</footer>